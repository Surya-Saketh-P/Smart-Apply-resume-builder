...
import { Sparkles, Save, AlertCircle, RefreshCw } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-2">
          ...
          <Card className="p-8 border-l-4 border-l-black space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={companyName} onChange={(e) => setCompanyName(e.target.value)} className="w-full border border-black px-3 py-2.5 bg-white focus:outline-none focus:bg-zinc-100 transition-all" placeholder="Google"/>
              </div>
              <div>
                <label className="block text-sm font-bold text-zinc-900 mb-2">Role</label>
                <input type="text" value={roleTitle} onChange={(e) => setRoleTitle(e.target.value)} className="w-full border border-black px-3 py-2.5 bg-white focus:outline-none focus:bg-zinc-100 transition-all" placeholder="Senior Engineer"/>
              </div>
            </div>

            <div>
              <label className="block text-sm font-bold text-zinc-900 mb-2">Job Description</label>
              <textarea rows={14} value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} className="w-full border border-black px-3 py-2.5 bg-white focus:outline-none focus:bg-zinc-100 transition-all resize-none font-mono text-xs leading-relaxed" placeholder="Paste the job description here..."/>
            </div>

            {error && (<div className="flex items-start gap-3 border-l-4 border-l-red-900 bg-white p-4 text-sm text-red-900">
                <AlertCircle className="h-4 w-4 shrink-0 mt-0.5"/>
                <span>{error}</span>
              </div>)}

            <Button onClick={handleAnalyze} disabled={analyzing} className="w-full py-3.5 text-base font-bold">
              {analyzing ? (<>
        ...
        <section className="space-y-8">
          <AnimatePresence mode="wait">
            {!result && !analyzing && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center h-full min-h-[500px] border-2 border-black bg-[#FAF9F6] p-12 text-center">
            ...
            {analyzing && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            ...
            {result && (<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6 pb-12">
                ...
                <Card className="p-6 border-t-4 border-t-black">
                  ...
                  <ul className="space-y-3">
                    {result.resumeBullets.map((bullet, i) => (<li key={i} className="flex gap-3 text-sm text-zinc-900 bg-white p-3 border border-black">
