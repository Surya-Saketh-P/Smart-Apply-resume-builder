...
import { Target, ListChecks, Plus, Rocket, Star, Flame } from 'lucide-react';
...
const Dashboard = () => {
    ...
    return (<div className="mx-auto max-w-7xl px-6 py-12">
      ...
      {!stats ? (<Card className="p-16 flex flex-col items-center justify-center text-center border-2 border-black">
        ...
        </Card>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12 divide-x divide-black border border-black">
      ...
      {stats && (<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <Card className="lg:col-span-2 p-8 border-l-4 border-l-black">
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={stats.chartData} cx="50%" cy="50%" innerRadius={80} outerRadius={120} paddingAngle={5} dataKey="value">
                    {stats.chartData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color}/>))}
                  </Pie>
                  <RechartsTooltip contentStyle={{ borderRadius: '0', border: '1px solid black', boxShadow: '2px 2px 0 rgba(0,0,0,1)' }}/>
                </PieChart>
              </ResponsiveContainer>
            </div>
            <div className="flex flex-wrap items-center justify-center gap-6 -mt-8 pt-4 border-t border-black">
                {stats.chartData.map(item => (<div key={item.name} className="flex items-center gap-2">
          ...
          <Card className="p-8 border-r-4 border-r-black">
            ...
            <div className="space-y-4 mb-8">
              {applications.slice(0, 4).map(app => (<div key={app.id} className="flex items-center justify-between p-3 border border-black hover:bg-zinc-100 transition-colors">
