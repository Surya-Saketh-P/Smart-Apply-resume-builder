...
import { Search, History as HistoryIcon, XCircle } from 'lucide-react';
    ...
    return (<div className="mx-auto max-w-7xl px-6 py-12">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-8">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search company or role..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border border-black bg-white focus:outline-none focus:bg-zinc-100 transition-all"/>
        </div>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-4 py-2.5 border border-black bg-white focus:outline-none focus:bg-zinc-100 transition-all appearance-none cursor-pointer font-medium">
      ...
      {!filteredApps || filteredApps.length === 0 ? (<Card className="flex flex-col items-center justify-center p-16 text-center border-2 border-black">
          ...
          {filteredApps.map((app) => (<ApplicationCard key={app.id} application={app} onDelete={deleteApplication} onStatusUpdate={updateStatus} onView={setSelectedApp}/>))}
        </div>)}

      <AnimatePresence>
        {selectedApp && (<div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4 overflow-y-auto">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white w-full max-w-4xl border-2 border-black relative my-auto">
              <button onClick={() => setSelectedApp(null)} className="absolute top-4 right-4 p-2 hover:bg-zinc-100 transition-colors z-10 text-zinc-900">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="border-t border-black pt-6">
                      ...
                      <ul className="space-y-2">
                        {selectedApp.resumeBullets.map((b, i) => (<li key={i} className="text-xs text-zinc-700 flex gap-2">
