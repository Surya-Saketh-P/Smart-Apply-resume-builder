...
export const Card = ({ children, className }) => (<div className={cn("border border-black bg-[#FAF9F6] overflow-hidden", className)}>
    {children}
  </div>);
export const Button = ({ children, className, variant = 'primary', ...props }) => {
    ...
    return (<button className={cn("inline-flex items-center justify-center gap-2 rounded-none px-4 py-2.5 text-sm font-medium transition-all focus:outline-none focus:border-black disabled:opacity-50 disabled:cursor-not-allowed", variants[variant], className)} {...props}>
      {children}
    </button>);
};
export const Badge = ({ children, color = 'gray' }) => {
    ...
    return (<span className={cn("inline-flex items-center rounded-none border px-2.5 py-0.5 text-xs font-bold", colors[color])}>
      {children}
    </span>);
};
