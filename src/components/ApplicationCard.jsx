...
import { Building2, Calendar, Trash2, ExternalLink } from 'lucide-react';
import { format } from 'date-fns';
const ApplicationCard = ({ application, onStatusUpdate, onDelete, onView }) => {
    const statusColors = {
        Applied: 'blue',
        Interview: 'orange',
        Offer: 'green',
    ...
    return (<Card className="p-6 border-l-4 border-l-black group">
      ...
      <div className="mt-6 pt-4 border-t border-black flex items-center justify-between">
        <div className="flex items-center gap-2">
          <select value={application.status} onChange={(e) => onStatusUpdate(application.id, e.target.value)} className="text-xs font-bold bg-white border border-black py-1.5 px-2.5 focus:outline-none cursor-pointer hover:bg-zinc-100 transition-colors">
        ...
        <div className="flex items-center gap-0 divide-x divide-black border border-black overflow-hidden">
          <button onClick={() => onView(application)} className="p-2 bg-white hover:bg-zinc-100 text-zinc-900 transition-colors" title="View Details">
            <ExternalLink className="h-4 w-4"/>
          </button>
          <button onClick={() => onDelete(application.id)} className="p-2 bg-white hover:bg-red-50 text-red-900 transition-colors" title="Delete">
